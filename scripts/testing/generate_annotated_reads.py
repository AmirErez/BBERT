...
from bbert.utils.common import setup_logging
...
def load_genome_sequences(fasta_file: str, logger: Logger) -> Dict[str, str]:
    """
    Load genome sequences from FASTA file (supports .gz compression).

    Args:
        fasta_file: Path to genome FASTA file (can be .gz compressed)
    ...
    genome = {}
    open_fn, fmt = detect_file_format(fasta_file)
    with open_fn(fasta_file) as f:
...
def generate_coding_reads_with_frames(fasta_file, gff_file, reads_per_cds=2, read_length=100, is_bacterial=True):
    ...
    # Handle compressed GTF/GFF files
...
def generate_noncoding_reads(fasta_file, gff_file, num_reads, read_length=100, is_bacterial=True):
    ...
    # Read genome sequence (supports .gz compression)
    genome = load_genome_sequences(fasta_file, logger)
